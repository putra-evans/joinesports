...
class Pubg extends CI_Controller
{
    ...
    {
        $this->template->load('vhome', 'components/pubg');
    }
    public function player()
    {
        $datas["players"] = $this->player_fmodel->get_pubg();
        $regis = $_SESSION['akun'];
        $username = $regis['nama'];
        $datas["id"] = $this->player_fmodel->idregis($username);
        $datas["cek"] = $this->player_fmodel->cekpubg();
        $datas["playerss"] = $this->player_fmodel->getAll_pubg();
        $this->template->load('vhome', 'components/playerpubg', $datas);
    }
    public function team()
    {
        ...
        $datas["team"] = $this->tim_fmodel->getpubg();
        $datas["cek2"] = $this->tim_fmodel->cek2();
        $this->template->load('vhome', 'components/teampubg', $datas);
    }
    public function tournaments()
    {
        $data['tour'] = $this->tournament_fmodel->getpubg();
        $data['tournament'] = $this->tournament_fmodel->getpubg();
        $this->template->load('vhome', 'components/tournamentspubg', $data);
    }
    ...
    {
        if (!isset($id)) redirect('components/playerpubg');

        $player = $this->player_fmodel;
        $validation = $this->form_validation;
        $validation->set_rules($player->rules());

        $data["players"] = $player->getByIdpubg($id);
        $data["player"] = $player->get_joinpubg($id);
    ...
    public function daftartournaments($id)
    {
        ...
        $idplayer = $this->tim_fmodel->cekidpubg();
        // $idplayer = $tournament->playeridpubg();
    ...
    public function request($id)
    {
        ...
        $validation = $this->form_validation;
        $validation->set_rules($tim->rules());
        $data['request'] = $tim->requestpubg($id);
    ...
    public function ApiContent()
    {
        $kode = $_POST['kode'];
        if ($kode == 1) {
            $data = $this->tim_fmodel->berdasarkannamapubg();
           ...
           <div class='row mt-3'>";
            ...
            $data = $this->tim_fmodel->berdasarkantahunpubg();
            echo " <div class='container'>
            <div class='row mt-3'>";
            foreach ($data as $a) {
                echo "
