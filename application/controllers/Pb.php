...
class Pb extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->template->load('vhome', 'components/pb', $data);
    }
    public function match()
    {
        ...
        $this->template->load('vhome', 'components/matchpb', $data);
    }
    public function player()
    {
        $datas["players"] = $this->player_fmodel->get_pb();
        $regis = $_SESSION['akun'];
        $username = $regis['nama'];
        $datas["id"] = $this->player_fmodel->idregis($username);
        $datas["cek"] = $this->player_fmodel->cekpb();
        $datas["playerss"] = $this->player_fmodel->getAll_pb();
        $this->template->load('vhome', 'components/playerpb', $datas);
    }
    public function team()
    {
        ...
        $datas["team"] = $this->tim_fmodel->getpb();
        $datas["cek2"] = $this->tim_fmodel->cek2();
        $this->template->load('vhome', 'components/teampb', $datas);
    }
    public function tournaments()
    {
        $data['tour'] = $this->tournament_fmodel->getpb();
        $data['tournament'] = $this->tournament_fmodel->getpb();
        $this->template->load('vhome', 'components/tournamentspb', $data);
    }
    ...
    {
        if (!isset($id)) redirect('components/playerpb');

        $player = $this->player_fmodel;
        $validation = $this->form_validation;
        $validation->set_rules($player->rules());

        $data["players"] = $player->getByIdpb($id);
        $data["player"] = $player->get_joinpb($id);
    ...
    public function daftartournaments($id)
    {
        ...
        $idplayer = $this->tim_fmodel->cekidpb();
        // $idplayer = $tournament->playeridpb();
    ...
    public function ApiContent()
    {
        $kode = $_POST['kode'];
        if ($kode == 1) {
            $data = $this->tim_fmodel->berdasarkannamapb();
           ...
           <div class='row mt-3'>";
            ...
            $data = $this->tim_fmodel->berdasarkantahunpb();
            echo " <div class='container'>
            <div class='row mt-3'>";
            foreach ($data as $a) {
                echo "
