...
<div class="container top-section" style=" margin-top: 120px">
    <div class=" d-flex flex-row mb-2 portal-nav">
        <a class="" href="http://joinesports.net/pb">
            <img src="<?php echo base_url('frontend/img/games/pb.png') ?>" alt="" height="30px" class="mr-2 mb-3"></a>
        ...
        <div class="portal-nav ml-auto">
            ...
            <a href="<?php echo base_url() ?>pb/tournaments" class="active">TOURNAMENTS</a>
        </div>
    </div>
</div>
<div class="container">
    <div class="row">
        <div class="col-lg-7">
            <div class="card bg-dark text-white mb-3">
                <img src="<?php echo base_url('frontend/img/pb.jpg') ?> " class="card-img" style="width: 633px; height: 280px">
        ...
        <div class="col-lg-5">
            <div class="card bg-black mb-3">
                <div class="container">
                    ...
                    $ambil = $koneksi->query("SELECT * FROM tb_tournament  where tournament_status='mulai' AND tournament_jenis='pb'");
    ...
    <div class="container">
        <table class="table table-borderless table-esport">
            ...
            <tbody>
                ...
                $koneksi = mysqli_connect('localhost', 'root', '', 'db_join');
                $now = date('Y-m-d');
                $ambil = $koneksi->query("SELECT * FROM tb_tournament LEFT JOIN tb_achievement on tb_tournament.tournament_id=tb_achievement.achievement_idtournament where tournament_status='segera' AND tournament_jenis='pb' ORDER BY tournament_tglmulai DESC");
                ...
                ?>
                        <tr>
                            <td></td>
                            <td style="color: #e0b403"><a href="<?php echo site_url('pb/detailtournament/' . $satu->tournament_id) ?>"> <?php echo $satu->tournament_nama ?></a> </td>
                            ...
                            if ($satu->tournament_participan == $total['jumlah']) {  ?>
                                ...
                                <td class="text-primary">
                                    <a href="<?php echo site_url('pb/detailtournament/' . $satu->tournament_id) ?>">OPEN FOR REGISTRATION</a>
                                </td> <?php } ?>
                        </tr>
                <?php }
                } ?>
            ...
            <tbody>
                <?php foreach ($tour as $tournament) :
                    if (($tournament->tournament_status == 'selesai') && ($tournament->tournament_id == $tournament->achievement_idtournament)) {  ?>
                        ...
                        <tr>
                            <td></td>
                            <td style="color: #e0b403"><a href="<?php echo site_url('pb/detailtournament/' . $tournament->tournament_id) ?>"><?php echo $tournament->tournament_nama ?></a></td>
                            <td><?php echo $tournament->tournament_participan ?> </td>
                            <td><?php echo $tournament->tournament_mode ?></td>
                            <td><?php echo $tournament->tournament_lokasi ?></td>
                            <td><?php echo $tournament->tournament_tglmulai ?></td>
                            <td><?php echo $tournament->tournament_tglakhir ?></td>
                            <td class="text-primary"><img src="<?php echo base_url('upload/tim/' . $tournament->tim_image) ?>" style="width: 40px;height: 40px" />&nbsp; <a href="<?php echo site_url('pb/detailtim/' . $tournament->tim_id) ?>"><?php echo $tournament->tim_nama ?></a></td>
                        </tr>
                <?php }
                endforeach; ?>
            </tbody>
